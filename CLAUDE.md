...
| predict-round | **Fable 5 ensemble** (the betting edge) | 3× `round-advisor` (lenses: stats / contrarian / conditions-market) → 1× `round-decider`, all Fable 5 |
...
| predict-top-scorers | **Fable 5** (single pass) | `top-scorer-picker` subagent — position-weighted scorer basket (re-picked each knockout phase) |
...
| predict-knockout | **Fable 5 ensemble** (the betting edge) | 4× `round-advisor` (+ `knockout-context` lens) → 1× `round-decider`, all Fable 5 |
...
**Prediction is an ensemble, not a single call.** Per round: three Fable 5 `round-advisor`s each predict the full round through a distinct lens and persist `state/advice/round-<n>/<lens>.md`; one Fable 5 `round-decider` then makes the final call by free, value-tilted synthesis (may pick a scoreline no advisor proposed) and writes the canonical `state/predictions/round-<n>.md`. predict-round stays all-Fable 5 regardless: it is where poule points are won.

**The two bonus bets are single-pass (not ensembles).** After the group stage, `champion-picker` (the 250-pt champion, on **Opus**) and `top-scorer-picker` (the position-weighted scorer basket, on **Fable 5**) each run as **one** subagent — no advisor panel, since they're once-off value bets rather than the per-round card. Both build on the group-stage result: the champion walks the bracket `scripts/knockout.py` seeds from the final standings, and the scorer basket is capped by each team's predicted goals.
